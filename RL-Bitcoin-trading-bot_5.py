...
class CustomEnv:
    ...
    def __init__(
        ...
        self.Show_indicators = (
            show_indicators  # show main indicators in rendered visualization
        )
...
def random_games(env, visualize, test_episodes=50, comment=''):
    ...
    for episode in range(test_episodes):
        ...
        while True:
            ...
            if env.current_step == env.end_step:
                ...
                print(
                    'episode: {}, net_worth: {}, average_net_worth: {}, orders: {}'.format(
                        episode,
                        env.net_worth,
                        average_net_worth / (episode + 1),
                        env.episode_orders,
                    )
                )
                break
    ...
    print(
        'average {} episodes random net_worth: {}, orders: {}'.format(
            test_episodes,
...
def train_agent(
    env, agent, visualize=False, train_episodes=50, training_batch_size=500
):
    agent.create_writer(
        env.initial_balance, env.normalize_value, train_episodes
    )  # create TensorBoard writer
    ...
    for episode in range(train_episodes):
        ...
        print(
            'episode: {:<5} net worth {:<7.2f} average: {:<7.2f} orders: {}'.format(
...
def test_agent(
        ...
        while True:
            ...
            if env.current_step == env.end_step:
                ...
                if env.net_worth < env.initial_balance:
                    no_profit_episodes += 1  # calculate episode count where we had negative profit through episode
                print(
                    'episode: {:<5}, net_worth: {:<7.2f}, average_net_worth: {:<7.2f}, orders: {}'.format(
                        episode,
                        env.net_worth,
                        average_net_worth / (episode + 1),
                        env.episode_orders,
    ...
    print(
        'average {} episodes agent net_worth: {}, orders: {}'.format(
            test_episodes,
            average_net_worth / test_episodes,
            average_orders / test_episodes,
        )
